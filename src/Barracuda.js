class Barracuda extends CatchableFish {
  ...
  constructor(game, ctx, size, position, image, maxFrameX, maxFrameY, dieFrameX, dieFrameY, spriteFrameSize) {
    ...
    this._sw = this._spriteFrameSize.getWidth();   // 512
    this._sh = this._spriteFrameSize.getHeight();  // 128
    this._staggerFrame = ANIM_STAGGER_SLOW;
    this._driftSpeed   = BARRACUDA_DRIFT_SPEED;
  }
  ...
  static create(game, ctx, spec) {
    return new Barracuda(
      game, ctx, spec.size,
      new Point(
        Barracuda.randomSpawnX(game.getSize().getWidth(), spec.size.getWidth()),
        Barracuda.randomSpawnY(game.getSize().getHeight(), spec.size.getHeight())
      ),
  ...
  draw() {
    ...
    // sprite faces RIGHT; flip when direction is -1 (going left)
