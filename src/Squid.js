class Squid extends CatchableFish {
  ...
  constructor(game, ctx, size, position, image, maxFrameX, maxFrameY, dieFrameX, dieFrameY, spriteFrameSize) {
    ...
    this._driftSpeed   = SQUID_DRIFT_SPEED;
  }

  static randomSpawnY(canvasHeight, fishHeight, rng = Math.random) {
    const minY = WATER_SURFACE_Y + 100;  // same depth zone as SwordFish / HammerHeadShark
  ...
  static create(game, ctx, spec) {
    return new Squid(
      game, ctx, spec.size,
      new Point(
        Squid.randomSpawnX(game.getSize().getWidth(), spec.size.getWidth()),
        Squid.randomSpawnY(game.getSize().getHeight(), spec.size.getHeight())
      ),
